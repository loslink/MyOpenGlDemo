...
public abstract class TextureSurfaceRenderer implements Runnable
{
    ...
    private void initGL()
    {
        ...
        eglDisplay = egl.eglGetDisplay(EGL10.EGL_DEFAULT_DISPLAY);//物理显示器句柄
        ...
        eglSurface = egl.eglCreateWindowSurface(eglDisplay, eglConfig, texture, null);//texture为渲染的目的
        ...
        //该接口将申请到的display，draw（surface）和 context进行了绑定。也就是说，在context下的OpenGLAPI指令将draw（surface）
        // 作为其渲染最终目的地。而display作为draw（surface）的前端显示。调用后，当前线程使用的EGLContex为context。
